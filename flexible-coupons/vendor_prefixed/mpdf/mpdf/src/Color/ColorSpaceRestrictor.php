...
class ColorSpaceRestrictor
{
     ...
     * @param int $mode
     */
    public function __construct(\FlexibleCouponsVendor\Mpdf\Mpdf $mpdf, \FlexibleCouponsVendor\Mpdf\Color\ColorModeConverter $colorModeConverter, $mode)
    {
        $this->mpdf = $mpdf;
    ...
    private function restrictSpotColorSpace($c, &$PDFAXwarnings = [])
    {
        if (!isset($this->mpdf->spotColorIDs[$c[1]])) {
            throw new \FlexibleCouponsVendor\Mpdf\MpdfException('Error: Spot colour has not been defined - ' . $this->mpdf->spotColorIDs[$c[1]]);
        }
        if ($this->mpdf->PDFA) {
