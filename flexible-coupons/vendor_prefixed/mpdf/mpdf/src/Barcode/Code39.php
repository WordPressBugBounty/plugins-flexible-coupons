 ...
 */
class Code39 extends \FlexibleCouponsVendor\Mpdf\Barcode\AbstractBarcode implements \FlexibleCouponsVendor\Mpdf\Barcode\BarcodeInterface
{
    /**
     * @param string $code
    ...
    private function init($code, $printRatio, $extended, $checksum)
    {
        ...
        if ($code === \false) {
            throw new \FlexibleCouponsVendor\Mpdf\Barcode\BarcodeException(\sprintf('Invalid CODE39 barcode value "%s"', $code));
        }
        if ($checksum) {
            // checksum
