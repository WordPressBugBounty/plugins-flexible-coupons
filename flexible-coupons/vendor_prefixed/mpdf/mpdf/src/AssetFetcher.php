...
use FlexibleCouponsVendor\Mpdf\File\LocalContentLoaderInterface;
use FlexibleCouponsVendor\Mpdf\File\StreamWrapperChecker;
use FlexibleCouponsVendor\Mpdf\Http\ClientInterface;
use FlexibleCouponsVendor\Mpdf\Log\Context as LogContext;
use FlexibleCouponsVendor\Mpdf\PsrHttpMessageShim\Request;
use FlexibleCouponsVendor\Mpdf\PsrLogAwareTrait\PsrLogAwareTrait;
use FlexibleCouponsVendor\Psr\Log\LoggerInterface;
class AssetFetcher implements \FlexibleCouponsVendor\Psr\Log\LoggerAwareInterface, \FlexibleCouponsVendor\Mpdf\AssetFetcherInterface
{
    ...
    public function fetchDataFromPath($path, $originalSrc = null)
    {
        ...
        if ($wrapperChecker->hasBlacklistedStreamWrapper($path)) {
            throw new \FlexibleCouponsVendor\Mpdf\Exception\AssetFetchingException('File contains an invalid stream. Only ' . implode(', ', $wrapperChecker->getWhitelistedStreamWrappers()) . ' streams are allowed.');
        }
    ...
    public function fetchRemoteContent($path)
    {
        ...
        try {
            ...
            if ($this->mpdf->debug) {
                throw new \FlexibleCouponsVendor\Mpdf\MpdfException($message, 0, \E_ERROR, null, null, $e);
            }
