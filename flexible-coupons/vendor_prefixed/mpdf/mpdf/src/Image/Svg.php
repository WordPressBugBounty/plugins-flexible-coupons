...
class Svg
{
    ...
    private $inDefs;
    public function __construct(\FlexibleCouponsVendor\Mpdf\Mpdf $mpdf, \FlexibleCouponsVendor\Mpdf\Otl $otl, \FlexibleCouponsVendor\Mpdf\CssManager $cssManager, \FlexibleCouponsVendor\Mpdf\Image\ImageProcessor $imageProcessor, \FlexibleCouponsVendor\Mpdf\SizeConverter $sizeConverter, \FlexibleCouponsVendor\Mpdf\Color\ColorConverter $colorConverter, \FlexibleCouponsVendor\Mpdf\Language\LanguageToFontInterface $languageToFont, \FlexibleCouponsVendor\Mpdf\Language\ScriptToLanguageInterface $scriptToLanguage)
    {
        $this->mpdf = $mpdf;
        $this->otl = $otl;
        ...
        if (isset($this->txt_data[2])) {
            ...
            if ($this->mpdf->useKerning) {
                if ($this->mpdf->CurrentFont['haskernGPOS']) {
                    ...
                    $textvar = $textvar | \FlexibleCouponsVendor\Mpdf\Css\TextVars::FC_KERNING;
                }
            }
    ...
    {
        $data = \preg_replace('/^.*?<svg([> ])/is', 'FlexibleCouponsVendor\\<svg\\1', $data);
        // mPDF 5.7.4
    ...
    function markScriptToLang($html)
    {
        ...
        foreach ($a as $i => $e) {
            if ($i % 2 == 0) {
                $e = \FlexibleCouponsVendor\Mpdf\Utils\UtfString::strcode2utf($e);
                ...
                for ($sch = 0; $sch <= $subchunk; $sch++) {
                    if (isset($chardata[$sch])) {
                        ...
                        for ($j = 0; $j < \count($chardata[$sch]); $j++) {
                            $s .= \FlexibleCouponsVendor\Mpdf\Utils\UtfString::code2utf($chardata[$sch][$j]['uni']);
                        }
                        ...
                        if ($scriptblocks[$sch] == \FlexibleCouponsVendor\Mpdf\Ucdn::SCRIPT_LATIN && $this->mpdf->autoVietnamese && \preg_match("/([" . $this->scriptToLanguage->getLanguageDelimiters('viet') . "])/u", $s)) {
                            $lang = "vi";
                        } elseif ($scriptblocks[$sch] == \FlexibleCouponsVendor\Mpdf\Ucdn::SCRIPT_ARABIC && $this->mpdf->autoArabic) {
                            if (\preg_match("/[" . $this->scriptToLanguage->getLanguageDelimiters('sindhi') . "]/u", $s)) {
                                ...
                                $lang = "fa";
                            } elseif ($this->mpdf->baseScript != \FlexibleCouponsVendor\Mpdf\Ucdn::SCRIPT_ARABIC && $this->scriptToLanguage->getLanguageByScript($scriptblocks[$sch])) {
    ...
    function xml_svg2pdf_start($parser, $name, $attribs)
    {
        ...
        switch (\strtolower($name)) {
            ...
            case 'svg':
                ...
                break;
            case 'path':
                $path = \FlexibleCouponsVendor\Mpdf\Utils\Arrays::get($attribs, 'd', '');
