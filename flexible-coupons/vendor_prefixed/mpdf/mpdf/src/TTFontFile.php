...
class TTFontFile
{
    ...
    public function getMetrics($file, $fontkey, $TTCfontID = 0, $debug = \false, $BMPonly = \false, $useOTL = 0)
    {
        ...
        if ($version === 0x4f54544f) {
            throw new \FlexibleCouponsVendor\Mpdf\Exception\FontException(sprintf('Fonts with postscript outlines are not supported (%s)', $file));
        }
    ...
    function _getGSUBignoreString($flag, $MarkFilteringSet)
    {
        ...
        // Flag & 0x0010 = UseMarkFilteringSet
        if ($flag & 0x10) {
            throw new \FlexibleCouponsVendor\Mpdf\Exception\FontException("Font \"" . $this->fontkey . "\" contains MarkGlyphSets which is not supported");
