 ...
 */
class WooTemplateResolver implements \FlexibleCouponsVendor\WPDesk\View\Resolver\Resolver
{
    /** @var string */
    private $base_path;
    ...
    public function resolve($name, \FlexibleCouponsVendor\WPDesk\View\Renderer\Renderer $renderer = null) : string
    {
        $template = \wc_locate_template($name, '', $this->base_path);
        if ($template === '') {
            throw new \FlexibleCouponsVendor\WPDesk\View\Resolver\Exception\CanNotResolve("Cannot resolve template {$name}");
        }
        return $template;
    }
}
