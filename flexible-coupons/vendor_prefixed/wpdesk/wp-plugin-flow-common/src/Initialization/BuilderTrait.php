...
namespace FlexibleCouponsVendor\WPDesk\Plugin\Flow\Initialization;

use FlexibleCouponsVendor\WPDesk\PluginBuilder\Plugin\Activateable;
use FlexibleCouponsVendor\WPDesk\PluginBuilder\Plugin\Deactivateable;
...
trait BuilderTrait
{
     ...
     * @return SlimPlugin
     */
    private function build_plugin(\FlexibleCouponsVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    private function init_register_hooks(\FlexibleCouponsVendor\WPDesk_Plugin_Info $plugin_info, \FlexibleCouponsVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
        if ($plugin instanceof \FlexibleCouponsVendor\WPDesk\PluginBuilder\Plugin\Activateable) {
            \register_activation_hook($plugin_info->get_plugin_file_name(), [$plugin, 'activate']);
        }
        if ($plugin instanceof \FlexibleCouponsVendor\WPDesk\PluginBuilder\Plugin\Deactivateable) {
     ...
     * @param SlimPlugin $plugin
     */
    private function store_plugin(\FlexibleCouponsVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
        $storageFactory = new \FlexibleCouponsVendor\WPDesk\PluginBuilder\Storage\StorageFactory();
     ...
     * @param SlimPlugin $plugin
     */
    private function init_plugin(\FlexibleCouponsVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin $plugin)
    {
