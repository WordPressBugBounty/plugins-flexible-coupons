...
final class CouponSettings extends FieldSettingsTab
{
    ...
    protected function get_fields(): array
    {
        ...
        $fields = [(new Header())->set_label('')->set_description(sprintf(
            ...
            '</a>'
        ))))->get_field(), (new DisableFieldProAdapter(self::COUPON_CODE_PREFIX_FIELD, (new InputTextField())->set_name('')->set_label(esc_html__('Coupon code prefix', 'flexible-coupons'))->set_description(__('Define the prefix which will be used as a beginning of your coupon code. Leave empty if you don’t want to use the prefix. Use <code>{order_id}</code> shortcode if you want to use the order number.', 'flexible-coupons'))->add_class('form-table-field'), \true))->get_field(), (new DisableFieldProAdapter(self::COUPON_CODE_SUFFIX_FIELD, (new InputTextField())->set_name('')->set_label(esc_html__('Coupon code suffix', 'flexible-coupons'))->set_description(__('Define the suffix which will be used as a end of your coupon code. Leave empty if you don’t want to use the suffix. Use <code>{order_id}</code> shortcode if you want to use the order number.', 'flexible-coupons'))->add_class('form-table-field'), \true))->get_field(), (new DisableFieldProAdapter(self::COUPON_CODE_LENGTH_FIELD, (new InputNumberField())->set_name('')->set_label(esc_html__('Number of random characters', 'flexible-coupons'))->set_description(esc_html__('The number of random characters in the coupon code. Random characters will be used for generating unique coupon codes. Choose the number between 5 and 30.', 'flexible-coupons'))->set_default_value(5)->set_attribute('min', 5)->set_attribute('max', 30)->add_class('form-table-field'), \true))->get_field(), (new DisableFieldProAdapter(self::REGULAR_PRICE_FIELD, (new CheckboxField())->set_name('')->set_label(esc_html__('Coupon value', 'flexible-coupons'))->set_sublabel(esc_html__('Enable', 'flexible-coupons'))->set_description(esc_html__('Always use the regular price of the product for the coupon value.', 'flexible-coupons')), \true))->get_field(), (new DisableFieldProAdapter(self::SHOW_TIPS_FIELD, (new CheckboxField())->set_name('')->set_label(esc_html__('Show field tips', 'flexible-coupons'))->set_sublabel(esc_html__('Enable', 'flexible-coupons'))->set_description(esc_html__('Show tooltips for fields.', 'flexible-coupons')), \true))->get_field(), (new DisableFieldProAdapter(self::SHOW_TEXTAREA_COUNTER_FIELD, (new CheckboxField())->set_name('')->set_label(esc_html__('Show textarea counter', 'flexible-coupons'))->set_sublabel(esc_html__('Enable', 'flexible-coupons'))->set_description(esc_html__('Show character counter below textarea.', 'flexible-coupons')), \true))->get_field(), (new NoOnceField(SettingsForm::NONCE_ACTION))->set_name(SettingsForm::NONCE_NAME), (new SubmitField())->set_attribute('id', 'save_settings')->set_name('save_settings')->set_label(esc_html__('Save Changes', 'flexible-coupons'))->add_class('button-primary')];
