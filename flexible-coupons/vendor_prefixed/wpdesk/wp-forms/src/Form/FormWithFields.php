...
use FlexibleCouponsVendor\WPDesk\Forms\Form;
use FlexibleCouponsVendor\WPDesk\Persistence\Adapter\ArrayContainer;
...
class FormWithFields implements \FlexibleCouponsVendor\WPDesk\Forms\Form, \FlexibleCouponsVendor\WPDesk\Forms\ContainerForm, \FlexibleCouponsVendor\WPDesk\Forms\FieldProvider
{
     ...
     * @inheritDoc
     */
    public function add_field(\FlexibleCouponsVendor\WPDesk\Forms\Field $field)
    {
        $this->fields[] = $field;
    }
    /**
    ...
    {
        if (\is_array($data)) {
            $data = new \FlexibleCouponsVendor\WPDesk\Persistence\Adapter\ArrayContainer($data);
        }
        foreach ($this->fields as $field) {
            $data_key = $field->get_name();
            if ($data->has($data_key)) {
                ...
                } catch (\FlexibleCouponsVendor\WPDesk\Persistence\ElementNotExistsException $e) {
     ...
     * @return string
     */
    public function render_fields(\FlexibleCouponsVendor\WPDesk\View\Renderer\Renderer $renderer)
    {
        $content = '';
     ...
     * @inheritDoc
     */
    public function render_form(\FlexibleCouponsVendor\WPDesk\View\Renderer\Renderer $renderer)
    {
     ...
     * @inheritDoc
     */
    public function put_data(\FlexibleCouponsVendor\WPDesk\Persistence\PersistentContainer $container)
    {
        foreach ($this->get_fields() as $field) {
            $data_key = $field->get_name();
            if (empty($data_key)) {
