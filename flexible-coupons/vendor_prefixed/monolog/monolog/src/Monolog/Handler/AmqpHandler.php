...
use FlexibleCouponsVendor\Monolog\Formatter\JsonFormatter;
use FlexibleCouponsVendor\PhpAmqpLib\Message\AMQPMessage;
use FlexibleCouponsVendor\PhpAmqpLib\Channel\AMQPChannel;
use AMQPExchange;
/**
 * @phpstan-import-type Record from \Monolog\Logger
 */
class AmqpHandler extends \FlexibleCouponsVendor\Monolog\Handler\AbstractProcessingHandler
{
    /**
     ...
     */
    public function __construct($exchange, ?string $exchangeName = null, $level = \FlexibleCouponsVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if ($exchange instanceof \FlexibleCouponsVendor\PhpAmqpLib\Channel\AMQPChannel) {
    ...
    private function createAmqpMessage(string $data) : \FlexibleCouponsVendor\PhpAmqpLib\Message\AMQPMessage
    {
        $attributes = ['delivery_mode' => 2, 'content_type' => 'application/json'];
        if ($this->extraAttributes) {
            $attributes = \array_merge($attributes, $this->extraAttributes);
        }
        return new \FlexibleCouponsVendor\PhpAmqpLib\Message\AMQPMessage($data, $attributes);
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return new \FlexibleCouponsVendor\Monolog\Formatter\JsonFormatter(\FlexibleCouponsVendor\Monolog\Formatter\JsonFormatter::BATCH_MODE_JSON, \false);
    }
}
