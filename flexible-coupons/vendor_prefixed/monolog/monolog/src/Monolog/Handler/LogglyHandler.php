...
use FlexibleCouponsVendor\Monolog\Formatter\LogglyFormatter;
 ...
 */
class LogglyHandler extends \FlexibleCouponsVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(string $token, $level = \FlexibleCouponsVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\extension_loaded('curl')) {
            throw new \FlexibleCouponsVendor\Monolog\Handler\MissingExtensionException('The curl extension is needed to use the LogglyHandler');
        }
        $this->token = $token;
    ...
    protected function send(string $data, string $endpoint) : void
    {
        ...
        \FlexibleCouponsVendor\Monolog\Handler\Curl\Util::execute($ch, 5, \false);
    }
    protected function getDefaultFormatter() : \FlexibleCouponsVendor\Monolog\Formatter\FormatterInterface
    {
        return new \FlexibleCouponsVendor\Monolog\Formatter\LogglyFormatter();
    }
}
