...
use FlexibleCouponsVendor\PhpConsole\Connector;
use FlexibleCouponsVendor\PhpConsole\Handler as VendorPhpConsoleHandler;
use FlexibleCouponsVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \FlexibleCouponsVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     */
    public function __construct(array $options = [], ?\FlexibleCouponsVendor\PhpConsole\Connector $connector = null, $level = \FlexibleCouponsVendor\Monolog\Logger::DEBUG, bool $bubble = \true)
    {
        if (!\class_exists('FlexibleCouponsVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(?\FlexibleCouponsVendor\PhpConsole\Connector $connector = null) : \FlexibleCouponsVendor\PhpConsole\Connector
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \FlexibleCouponsVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \FlexibleCouponsVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\FlexibleCouponsVendor\PhpConsole\Helper::isRegistered()) {
            \FlexibleCouponsVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \FlexibleCouponsVendor\PhpConsole\Handler::getInstance();
    ...
    public function getConnector() : \FlexibleCouponsVendor\PhpConsole\Connector
    {
    ...
    {
        if ($record['level'] < \FlexibleCouponsVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record) : void
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \FlexibleCouponsVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \FlexibleCouponsVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
