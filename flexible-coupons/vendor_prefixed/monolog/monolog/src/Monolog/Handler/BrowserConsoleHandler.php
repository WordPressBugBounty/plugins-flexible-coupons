 ...
 */
class BrowserConsoleHandler extends \FlexibleCouponsVendor\Monolog\Handler\AbstractProcessingHandler
{
    /** @var bool */
    ...
    {
        return new \FlexibleCouponsVendor\Monolog\Formatter\LineFormatter('[[%channel%]]{macro: autolabel} [[%level_name%]]{font-weight: bold} %message%');
    }
    /**
     * {@inheritDoc}
     */
    ...
    {
        return [\FlexibleCouponsVendor\Monolog\Logger::DEBUG => 'debug', \FlexibleCouponsVendor\Monolog\Logger::INFO => 'info', \FlexibleCouponsVendor\Monolog\Logger::NOTICE => 'info', \FlexibleCouponsVendor\Monolog\Logger::WARNING => 'warn', \FlexibleCouponsVendor\Monolog\Logger::ERROR => 'error', \FlexibleCouponsVendor\Monolog\Logger::CRITICAL => 'error', \FlexibleCouponsVendor\Monolog\Logger::ALERT => 'error', \FlexibleCouponsVendor\Monolog\Logger::EMERGENCY => 'error'][$level] ?? 'log';
    }
    /**
     * @return string[]
     */
    private static function handleStyles(string $formatted) : array
    {
        ...
        foreach (\array_reverse($matches) as $match) {
            ...
            $pos = $match[0][1];
            $format = \FlexibleCouponsVendor\Monolog\Utils::substr($format, 0, $pos) . '%c' . $match[1][0] . '%c' . \FlexibleCouponsVendor\Monolog\Utils::substr($format, $pos + \strlen($match[0][0]));
        }
    ...
    private static function handleCustomStyles(string $style, string $string) : string
    {
        ...
        if (null === $style) {
            ...
            throw new \RuntimeException('Failed to run preg_replace_callback: ' . $pcreErrorCode . ' / ' . \FlexibleCouponsVendor\Monolog\Utils::pcreLastErrorMessage($pcreErrorCode));
        }
        return $style;
    }
    /**
