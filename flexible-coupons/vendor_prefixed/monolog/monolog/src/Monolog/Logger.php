 ...
 */
class Logger implements \Psr\Log\LoggerInterface, \FlexibleCouponsVendor\Monolog\ResettableInterface
{
    /**
     ...
     */
    public function pushHandler(\FlexibleCouponsVendor\Monolog\Handler\HandlerInterface $handler) : self
    {
     ...
     */
    public function popHandler() : \FlexibleCouponsVendor\Monolog\Handler\HandlerInterface
    {
        if (!$this->handlers) {
     ...
     */
    public function addRecord(int $level, string $message, array $context = [], ?\FlexibleCouponsVendor\Monolog\DateTimeImmutable $datetime = null) : bool
    {
        ...
        try {
            ...
            foreach ($this->handlers as $handler) {
                if (null === $record) {
                    ...
                    $record = ['message' => $message, 'context' => $context, 'level' => $level, 'level_name' => $levelName, 'channel' => $this->name, 'datetime' => $datetime ?? new \FlexibleCouponsVendor\Monolog\DateTimeImmutable($this->microsecondTimestamps, $this->timezone), 'extra' => []];
                    try {
    ...
    public function reset() : void
    {
        ...
        foreach ($this->processors as $processor) {
            if ($processor instanceof \FlexibleCouponsVendor\Monolog\ResettableInterface) {
                $processor->reset();
            }
        }
    }
